...
void onEvent(ev_t event) {
    //DEBUG_PORT.print("TTN Event: ");
    //DEBUG_PORT.println(event);
    switch(event) {
        case EV_JOIN_FAILED:
          screen_print("ttn", "EV_JOIN_FAILED");
          //DEBUG_PORT.println("Join failed (check MIC / keys)");
          break;
        case EV_JOINING:
            screen_print("ttn", "EV_JOINING");
            //DEBUG_PORT.println("Joining TTN...");
            // Watchdog feed during join
            esp_task_wdt_reset();
            break;
        case EV_JOINED: {
            //DEBUG_PORT.println("Joined!");
        ...
        case EV_TXCOMPLETE:
            ...
            if (LMIC.dataLen) {
                ...
                DEBUG_PORT.println();
                DEBUG_PORT.print("Downlink received on ");
                if (LMIC.txrxFlags & TXRX_DNW1) DEBUG_PORT.print("RX1 ");
                if (LMIC.txrxFlags & TXRX_DNW2) DEBUG_PORT.print("RX2 ");
                DEBUG_PORT.print(" DR="); DEBUG_PORT.print(LMIC.datarate);
                DEBUG_PORT.print(" Freq="); DEBUG_PORT.println(LMIC.freq);
...
void ttn_join() {
    ...
    if(!keysgood) {
        ...
        screen_print("ttn", "No session saved, joining from scratch");
        LMIC_startJoining();
    } else {
        screen_print("ttn", "Rejoining saved session");
...
static void ttn_set_cnt() {
    ...
    uint32_t now = millis();
    if(now < lastWriteMsec || (now - lastWriteMsec) > COUNT_SAVE_EVERY_M * 60 * 1000L) { // write if we roll over (50 days) or 5 mins
        lastWriteMsec = now;
